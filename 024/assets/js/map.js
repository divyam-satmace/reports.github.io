mapboxgl.accessToken = 'pk.eyJ1Ijoic2F0bWFjZSIsImEiOiJjbGdrb281bDQwYXM3M2pwcGxhOWt2bTJ3In0.bHEg6Y0rVuU1eoGLX3hLWg';

var mapContainer = document.getElementById("mapbox1");
var infoBox = document.getElementById("infoBox");
var map = new mapboxgl.Map({
    container: mapContainer,
    style: 'mapbox://styles/mapbox/streets-v12',
    center: [73.856255, 20.716726],
    zoom: 6
});

//73.869400, 19.205700
//73.130539, 19.240330

var markers = [
    {
        lngLat: [72.9968 , 20.50095],
        popupContent: '<h3>Pune, Maharashtra</h3><p>Collection centre</p>',
        backgroundColor: '#003E46', // Background color for the marker,
        popupContent: '<img src="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEhkOW21GDuNAoWkvOW0ULGamA42SRELQV-teGlX7yd03eQahi4Hhc9V36--rVnSIyoNLai2QxRy7-SeCkhPcYd-gy15GVd4cDbufHP3lNlNzqd01-q0cBFY7HcOzg_CuqboR6lNyMiN5jUrooiFmIB8gci30IbJlDtHz2y68DR5FgnglaJ45A9GXlwiuqw/w541-h308/umbergaon.jpg" style="max-width: 100%;">, <h3>Vankas, Gujrat</h3><p>Recycling centre </p>',
        imageUrl: 'https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEg4he-Y0Hbs1l3NTg_FqIohB6sjmNg1jnvu9L3-P2EC5M7OE5Pue85-N-cP-O1HeRRm5de8-surCkkclZT4GpS-KD1SK6elHWedX0jWX_fk9xi92Pz0z2fOMAd8Lv60rIPJbumCwY3NgDDHKvrFsIXEgjNjRTfiBWTyU-6-cXVAbJnwP9p4EmT0lc95Oq0/s25/recycle-white-icon.png' // URL for the PNG image
    },
    // {
    //     lngLat: [73.130539, 19.240330],   //769DFA
    //     popupContent: '<h3>Ahmednagar, Maharashtra</h3><p>Collection centre</p>',
    //     backgroundColor: '#769DFA', // Background color for the marker
    //     imageUrl: 'https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEhmafP0XyqK8WNXvdyHtAYwusgo-zOPZxPqU9dX5w7mcjl19I-h-5xjp1JStUs2qV5B4tF-NvTjR1caNDQHKYkYuMa1Vdi2bknPQVqN9hnFWdupiqpRCff7txx8bXsqiNsC95vqEkOC0l_9u4TmJQiURpjKjs8NrTA3sVj_4tKfbULblbl0pPeI4h-8oW8/s20/product-map-icon.png' // URL for the PNG image
    // },
    {
        lngLat: [72.9968 , 20.90095],
        popupContent: '<h3>Kalyan, Maharashtra</h3><p>Collection centre</p>',
        backgroundColor: '#59BE8A', // Background color for the marker
        popupContent: '<img src="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjPUjLXke6KRAn6ubBHxGNVu20-eP8WBIVpxKH587vf0znKI_T6iXMdO0krGyPLbGYTm4HWezJ4QugHaCH3CQQluJhdgLNMUZRfQkCY51QMMb0hS2UEovPxPgJd6pdcwEjdO5SkS2Fb5lacKJ-UXyKNCV7d9igtwJf2vH0y6iJrE-AQNtEXRA2yXL1CDpQ/s320/collection-1.jpeg"  width="180" height="100" ><h3>Vapi , Gujrat</h3><p>Collection centre </p>',
        imageUrl: 'https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjv_nj0aiw0reUJBTXSe1yj-Ben6QruQFFQz8Q66gETfZhxsFUCY3fT1PiWFiDqyq2ttKErPLEdVyq1PVOOMdGNFI4wL5hbnA0LpYvjR8_-EcDLMoVmOi8zKtISqP3t9JZISiZdf3UIhFGW_k5ZQnKvHEpqh08LC0upqBMlKSidCrNGErViwSpyX7D8Tt4/s23/collecyion-map-icon.png' // URL for the PNG image
    },
    {
        lngLat: [76.9049 , 28.3725],
        popupContent: '<h3>Junnar, Maharashtra</h3><p>Collection centre</p>',
        backgroundColor: '#59BE8A', // Background color for the marker
        popupContent: '<img src="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEhEc6vLmx5OA-0_r4eBVP1FJxVaKT0HEtkv8JrfTuqqKRV-pO9bh3MdXLY6eRf1EYsF_Q196LND_D9AeFFIYdRpM_hgm1vnENH42ZpuTcF2B2xiinURnr2aHNZO_ckJ_8crpQ5nszDU4Pq0I2b9PBpR5k0-gIJGlBk5ne_97pjpyqSVzvX05KxiamQzNKU/s1026/test.jpeg" width="150" height="100" ><h3>Manesar, Haryana</h3><p>Collection centre </p>',
        imageUrl: 'https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjv_nj0aiw0reUJBTXSe1yj-Ben6QruQFFQz8Q66gETfZhxsFUCY3fT1PiWFiDqyq2ttKErPLEdVyq1PVOOMdGNFI4wL5hbnA0LpYvjR8_-EcDLMoVmOi8zKtISqP3t9JZISiZdf3UIhFGW_k5ZQnKvHEpqh08LC0upqBMlKSidCrNGErViwSpyX7D8Tt4/s23/collecyion-map-icon.png' // URL for the PNG image
    },
    // Add more markers here if needed
];

markers.forEach(function (markerInfo) {
    var marker = new mapboxgl.Marker({
        element: createCustomMarker(markerInfo.backgroundColor, markerInfo.imageUrl)
    })
    .setLngLat(markerInfo.lngLat)
    .addTo(map);

    // var popup = new mapboxgl.Popup({ className: 'marker-popup' }).setHTML(markerInfo.popupContent);

    marker.getElement().addEventListener('click', function () {
        if (marker.getPopup()) {
            marker.setPopup(null);
        } else {
            marker.setPopup(popup);
        }
    });
});

// Function to create a custom marker with background color and an image
function createCustomMarker(backgroundColor, imageUrl) {
    var markerElement = document.createElement('div');
    markerElement.className = 'custom-marker';
    markerElement.style.backgroundColor = backgroundColor;

    var imageElement = document.createElement('img');
    imageElement.src = imageUrl;
    markerElement.appendChild(imageElement);

    return markerElement;
}
// map.addControl(new mapboxgl.NavigationControl(),'bottom-left');
// map.addControl(new mapboxgl.FullscreenControl());


var mapContainer2 = document.getElementById("mapbox2");
var map2 = new mapboxgl.Map({
    container: mapContainer2,
    style: 'mapbox://styles/mapbox/streets-v12',
    center: [73.856255, 20.716726],
    zoom: 6
});

// map2.on('load', function () {
//     var start = [72.877655, 19.075983];
//     var destinations = [
//         [74.7749, 20.9042],
//         [74.173257, 21.975842],
//         [73.243896, 22.264793],
//         // Add more destination points as needed
//     ];

//     var coordinates = [];
//     destinations.forEach(function (destination) {
//         coordinates.push(start);
//         coordinates.push(destination);
//     });

//     // Get route from Mapbox Directions API
//     var apiUrl = 'https://api.mapbox.com/directions/v5/mapbox/driving/';
//     apiUrl += coordinates.join(';');
//     apiUrl += '?steps=true&geometries=geojson&access_token=' + mapboxgl.accessToken;

//     fetch(apiUrl)
//         .then(function (response) {
//             return response.json();
//         })
//         .then(function (data) {
//             var route = data.routes[0].geometry;

//             map2.addLayer({
//                 id: 'route',
//                 type: 'line',
//                 source: {
//                     type: 'geojson',
//                     data: {
//                         type: 'Feature',
//                         geometry: route
//                     }
//                 },
//                 paint: {
//                     'line-color': '#55d5a5',
//                     'line-width': 4
//                 }
//             });

//             // Add marker at start point
//             var startMarker = new mapboxgl.Marker()
//                 .setLngLat(start)
//                 .addTo(map2);

//             // Add markers at destination points
//             destinations.forEach(function (destination) {
//                 var marker = new mapboxgl.Marker()
//                     .setLngLat(destination)
//                     .addTo(map2);
//             });

//             var bounds = new mapboxgl.LngLatBounds();
//             coordinates.forEach(function (coord) {
//                 bounds.extend(coord);
//             });
//             // map2.fitBounds(bounds, { padding: 40 });
//         });
// });
var markers = [
  
    // {
    //     lngLat: [73.889551, 18.444276], // [longitude, latitude] of the marker position
    //     imageUrl: 'https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjv_nj0aiw0reUJBTXSe1yj-Ben6QruQFFQz8Q66gETfZhxsFUCY3fT1PiWFiDqyq2ttKErPLEdVyq1PVOOMdGNFI4wL5hbnA0LpYvjR8_-EcDLMoVmOi8zKtISqP3t9JZISiZdf3UIhFGW_k5ZQnKvHEpqh08LC0upqBMlKSidCrNGErViwSpyX7D8Tt4/s23/collecyion-map-icon.png', // URL for the PNG image
    //     popupContent: '<img src="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEhEc6vLmx5OA-0_r4eBVP1FJxVaKT0HEtkv8JrfTuqqKRV-pO9bh3MdXLY6eRf1EYsF_Q196LND_D9AeFFIYdRpM_hgm1vnENH42ZpuTcF2B2xiinURnr2aHNZO_ckJ_8crpQ5nszDU4Pq0I2b9PBpR5k0-gIJGlBk5ne_97pjpyqSVzvX05KxiamQzNKU/s1026/test.jpeg" width="150" height="100" ><h3>Pune, Maharashtra</h3><p>Collection centre </p>'
    // },
    // {
    //     lngLat: [73.889551, 18.490276], // [longitude, latitude] of the marker position
    //     imageUrl: 'https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjv_nj0aiw0reUJBTXSe1yj-Ben6QruQFFQz8Q66gETfZhxsFUCY3fT1PiWFiDqyq2ttKErPLEdVyq1PVOOMdGNFI4wL5hbnA0LpYvjR8_-EcDLMoVmOi8zKtISqP3t9JZISiZdf3UIhFGW_k5ZQnKvHEpqh08LC0upqBMlKSidCrNGErViwSpyX7D8Tt4/s23/collecyion-map-icon.png', // URL for the PNG image
    //     popupContent: '<img src="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEhEc6vLmx5OA-0_r4eBVP1FJxVaKT0HEtkv8JrfTuqqKRV-pO9bh3MdXLY6eRf1EYsF_Q196LND_D9AeFFIYdRpM_hgm1vnENH42ZpuTcF2B2xiinURnr2aHNZO_ckJ_8crpQ5nszDU4Pq0I2b9PBpR5k0-gIJGlBk5ne_97pjpyqSVzvX05KxiamQzNKU/s1026/test.jpeg" width="150" height="100" ><h3>Pune, Maharashtra</h3><p>Collection centre </p>'
    // },

    // {
    //     lngLat: [74.767599, 19.089211], // [longitude, latitude] of the marker position
    //     imageUrl: 'https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjv_nj0aiw0reUJBTXSe1yj-Ben6QruQFFQz8Q66gETfZhxsFUCY3fT1PiWFiDqyq2ttKErPLEdVyq1PVOOMdGNFI4wL5hbnA0LpYvjR8_-EcDLMoVmOi8zKtISqP3t9JZISiZdf3UIhFGW_k5ZQnKvHEpqh08LC0upqBMlKSidCrNGErViwSpyX7D8Tt4/s23/collecyion-map-icon.png' ,// URL for the PNG image
    //     popupContent: '<img src="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjPUjLXke6KRAn6ubBHxGNVu20-eP8WBIVpxKH587vf0znKI_T6iXMdO0krGyPLbGYTm4HWezJ4QugHaCH3CQQluJhdgLNMUZRfQkCY51QMMb0hS2UEovPxPgJd6pdcwEjdO5SkS2Fb5lacKJ-UXyKNCV7d9igtwJf2vH0y6iJrE-AQNtEXRA2yXL1CDpQ/s320/collection-1.jpeg"  width="180" height="100" ><h3>Ahmednagar , Maharashtra</h3><p>Collection centre </p>'
    // },
    // {
    //     lngLat: [74.767599, 19.019211], // [longitude, latitude] of the marker position
    //     imageUrl: 'https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjv_nj0aiw0reUJBTXSe1yj-Ben6QruQFFQz8Q66gETfZhxsFUCY3fT1PiWFiDqyq2ttKErPLEdVyq1PVOOMdGNFI4wL5hbnA0LpYvjR8_-EcDLMoVmOi8zKtISqP3t9JZISiZdf3UIhFGW_k5ZQnKvHEpqh08LC0upqBMlKSidCrNGErViwSpyX7D8Tt4/s23/collecyion-map-icon.png' ,// URL for the PNG image
    //     popupContent: '<img src="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjPUjLXke6KRAn6ubBHxGNVu20-eP8WBIVpxKH587vf0znKI_T6iXMdO0krGyPLbGYTm4HWezJ4QugHaCH3CQQluJhdgLNMUZRfQkCY51QMMb0hS2UEovPxPgJd6pdcwEjdO5SkS2Fb5lacKJ-UXyKNCV7d9igtwJf2vH0y6iJrE-AQNtEXRA2yXL1CDpQ/s320/collection-1.jpeg"  width="180" height="100" ><h3>Ahmednagar , Maharashtra</h3><p>Collection centre </p>'
    // },
    {
        lngLat: [72.9968 , 20.90095], // [longitude, latitude] of the marker position
        imageUrl: 'https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjv_nj0aiw0reUJBTXSe1yj-Ben6QruQFFQz8Q66gETfZhxsFUCY3fT1PiWFiDqyq2ttKErPLEdVyq1PVOOMdGNFI4wL5hbnA0LpYvjR8_-EcDLMoVmOi8zKtISqP3t9JZISiZdf3UIhFGW_k5ZQnKvHEpqh08LC0upqBMlKSidCrNGErViwSpyX7D8Tt4/s23/collecyion-map-icon.png' ,// URL for the PNG image
        popupContent: '<img src="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjPUjLXke6KRAn6ubBHxGNVu20-eP8WBIVpxKH587vf0znKI_T6iXMdO0krGyPLbGYTm4HWezJ4QugHaCH3CQQluJhdgLNMUZRfQkCY51QMMb0hS2UEovPxPgJd6pdcwEjdO5SkS2Fb5lacKJ-UXyKNCV7d9igtwJf2vH0y6iJrE-AQNtEXRA2yXL1CDpQ/s320/collection-1.jpeg"  width="180" height="100" ><h3>Vapi , Maharashtra</h3><p>Collection centre </p>'
    },
    {
        lngLat: [76.9049 , 28.3725], // [longitude, latitude] of the marker position
        imageUrl: 'https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjv_nj0aiw0reUJBTXSe1yj-Ben6QruQFFQz8Q66gETfZhxsFUCY3fT1PiWFiDqyq2ttKErPLEdVyq1PVOOMdGNFI4wL5hbnA0LpYvjR8_-EcDLMoVmOi8zKtISqP3t9JZISiZdf3UIhFGW_k5ZQnKvHEpqh08LC0upqBMlKSidCrNGErViwSpyX7D8Tt4/s23/collecyion-map-icon.png' ,// URL for the PNG image
        popupContent: '<img src="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjPUjLXke6KRAn6ubBHxGNVu20-eP8WBIVpxKH587vf0znKI_T6iXMdO0krGyPLbGYTm4HWezJ4QugHaCH3CQQluJhdgLNMUZRfQkCY51QMMb0hS2UEovPxPgJd6pdcwEjdO5SkS2Fb5lacKJ-UXyKNCV7d9igtwJf2vH0y6iJrE-AQNtEXRA2yXL1CDpQ/s320/collection-1.jpeg"  width="180" height="100" ><h3>Manesar , Haryana </h3><p>Collection centre </p>'
    },

  
    // Add more markers here if needed
];

function updateInfoBox(content) {
    infoBox.innerHTML = content;
}


// Create markers and popups dynamically
markers.forEach(function (markerInfo) {
    var marker = new mapboxgl.Marker()
        .setLngLat(markerInfo.lngLat)
        .addTo(map2);

    var popup = new mapboxgl.Popup({ className: 'marker-popup' }).setHTML(markerInfo.popupContent);

    marker.getElement().addEventListener('click', function () {
        if (marker.getPopup()) {
            marker.setPopup(null);
        } else {
            marker.setPopup(popup);
        }
    });
});

function createCustomMarker(backgroundColor, imageUrl) {
    var markerElement = document.createElement('div');
    markerElement.className = 'custom-marker';
    markerElement.style.backgroundColor = backgroundColor;

    var imageElement = document.createElement('img');
    imageElement.src = imageUrl;
    markerElement.appendChild(imageElement);

    return markerElement;
}



// updateInfoBox("<h2>Collection</h2><p> <b> CLEAR HDPE &nbsp &nbsp &nbsp &nbsp &nbsp </b> </p>");

// map2.addControl(new mapboxgl.NavigationControl(),'bottom-left');
// map2.addControl(new mapboxgl.FullscreenControl());




var mapContainer3 = document.getElementById("mapbox3");
var infoBox3 = document.getElementById("infoBox3");
var map3 = new mapboxgl.Map({
    container: mapContainer3,
    style: 'mapbox://styles/mapbox/streets-v12',
    center: [73.856255, 20.716726],                                                   //[73.856255, 19.316726],
    zoom: 6
});


map3.on('load', function () {
    var start = [72.9968 , 20.50095 ];
    var destinations = [
        [73.889551, 18.444276],   //74.468330, 21.545584
        [74.767599, 19.089211],      //74.5089, 20.5579
        // [73.243896, 22.264793],
        // Add more destination points as needed
    ];

    

    var coordinates = [];
    destinations.forEach(function (destination) {
        coordinates.push(start);
        coordinates.push(destination);
    });
   

    // Get route from Mapbox Directions API
    var apiUrl = 'https://api.mapbox.com/directions/v5/mapbox/driving/';
    apiUrl += coordinates.join(';');
    apiUrl += '?steps=true&geometries=geojson&access_token=' + mapboxgl.accessToken;

    fetch(apiUrl)
        .then(function (response) {
            return response.json();
        })
        .then(function (data) {
            var route = data.routes[0].geometry;

            // map3.addLayer({
            //     id: 'route',
            //     type: 'line',
            //     source: {
            //         type: 'geojson',
            //         data: {
            //             type: 'Feature',
            //             geometry: route
            //         }
            //     },
            //     paint: {
            //         'line-color': '#55d5a5',
            //         'line-width': 4
            //     }
            // });

          

            // Add marker at start point
            var startMarker = new mapboxgl.Marker()
                .setLngLat(start)
                .addTo(map3)
                .setPopup(
                    new mapboxgl.Popup({
                        closeButton: true,
                        closeOnClick: false
                    }).setHTML(
                        '<div style="max-width: 200px;">' +
                        '<img src="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEhkOW21GDuNAoWkvOW0ULGamA42SRELQV-teGlX7yd03eQahi4Hhc9V36--rVnSIyoNLai2QxRy7-SeCkhPcYd-gy15GVd4cDbufHP3lNlNzqd01-q0cBFY7HcOzg_CuqboR6lNyMiN5jUrooiFmIB8gci30IbJlDtHz2y68DR5FgnglaJ45A9GXlwiuqw/w541-h308/umbergaon.jpg" style="max-width: 100%;">' +
                        '<h3>Vankas, Gujrat</h3>' +
                        '<p>Collection centre of Shrink Bag</p>' +
                        '</div>'
                    )
                );

            // Add markers at destination points
            // destinations.forEach(function (destination) {
            //     var marker = new mapboxgl.Marker({ color: '#808080' })
            //         .setLngLat(destination)
            //         .addTo(map3);
            // });

            
            
            var bounds = new mapboxgl.LngLatBounds();
            coordinates.forEach(function (coord) {
                bounds.extend(coord);
            });
            // map3.fitBounds(bounds, { padding: 40 });
        });
       
});
function updateInfoBox3(content) {
    infoBox3.innerHTML = content;
}


// updateInfoBox3("<h2>Recycling</h2><p> <b> CLEAR HDPE &nbsp&nbsp&nbsp &nbsp &nbsp </b> </p>");
// map3.addControl(new mapboxgl.NavigationControl());
// map3.addControl(new mapboxgl.FullscreenControl());




var mapContainer4 = document.getElementById("mapbox4");
var infoBox4 = document.getElementById("infoBox4");
var map4 = new mapboxgl.Map({
    container: mapContainer4,
    style: 'mapbox://styles/mapbox/streets-v12',
    center: [72.7474142, 20.1924098],
    zoom: 10
});

const end = [72.8309,20.2200];   //72.8309,20.2286
const start = [72.750702,20.193300];  //73.856743, 18.520430

const directionsRequest = `https://api.mapbox.com/directions/v5/mapbox/driving/${start[0]},${start[1]};${end[0]},${end[1]}?geometries=geojson&access_token=${mapboxgl.accessToken}`;

map4.on('load', async () => {
    const response = await fetch(directionsRequest);
    const data = await response.json();

    const geojson = {
        type: 'FeatureCollection',
        features: [
            {
                type: 'Feature',
                properties: {},
                geometry: data.routes[0].geometry
            }
        ]
    };

    map4.addSource('line', {
        type: 'geojson',
        data: geojson
    });

    // Add line layer
    map4.addLayer({
        type: 'line',
        source: 'line',
        id: 'line-layer',
        paint: {
            'line-color': '#55d5a5',
            'line-width': 4,
        }
    });

    new mapboxgl.Marker({ color: '#808080' })
        .setLngLat(start)
        .addTo(map4);
        

    new mapboxgl.Marker({ color: '#13BEE1' })
        .setLngLat(end)
        .addTo(map4)
        .setPopup(
            new mapboxgl.Popup({
                closeButton: true,
                closeOnClick: false
            }).setHTML(
                '<div style="max-width: 200px;">' +
                '<img src="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEi1pRRdft8_9-DpmIx5J4uwp5B4cteaS47atmq1v6DAwd0JiBJPH1CaL7L2EJ4DYbGK4H5rMZ-BjGDHP8URBajK_dAvQxmRuOam8jHRAoGFG8UzsMHAbB-TwCQMQ7kujVxanX2bq-2xf7p179Hs-BH8E_uwe8JH-ifskLIzgUJOVrnlYItZxwILIhO2q2w/s320/WhatsApp%20Image%202023-07-12%20at%202.55.09%20PM.jpeg" style="max-width: 100%;">' +
                '<h3>Tattva Enterprises LLP, Gujrat</h3>' +
                '<p>Collection centre </p>' +
                '</div>'
            )
        );

    // Add a moving object with a car icon 
    const objectMarker = new mapboxgl.Marker({
        element: createMarkerElement("./assets/images/truck.png"), // Replace 'car-icon.png' with your custom car icon image path
        anchor: 'bottom',
        rotationAlignment: 'map'
    })
        .setLngLat(start)
        .addTo(map4);

    let currentCoordinateIndex = 0;
    const coordinates = data.routes[0].geometry.coordinates;
    const totalDistance = data.routes[0].distance / 1000; // Convert distance to kilometers

    const distanceElement = document.createElement('div');
    distanceElement.className = 'distance';
    distanceElement.innerText = totalDistance.toFixed(2) + 'km';
    objectMarker.getElement().appendChild(distanceElement);

    function moveObject() {
        const startPoint = coordinates[currentCoordinateIndex];
        const endPoint = coordinates[currentCoordinateIndex + 1];

        const startTime = new Date().getTime();
        const duration = 600; // Animation duration in milliseconds

        function animateMarker() {
            const currentTime = new Date().getTime();
            const elapsed = currentTime - startTime;
            const progress = elapsed / duration;

            const lng = startPoint[0] + (endPoint[0] - startPoint[0]) * progress;
            const lat = startPoint[1] + (endPoint[1] - startPoint[1]) * progress;

            objectMarker.setLngLat([lng, lat]);

            if (progress < 1) {
                requestAnimationFrame(animateMarker);
            } else {
                currentCoordinateIndex++;

                if (currentCoordinateIndex >= coordinates.length - 1) {
                    currentCoordinateIndex = 0; // Reset to start point to start the loop again
                }

                distanceElement.innerText = ((currentCoordinateIndex * totalDistance) / (coordinates.length - 1)).toFixed(2) + ' km'; // Update distance above the truck icon
                moveObject(); // Move to the next segment of the line
            }
        }
        animateMarker();
    }

    moveObject();
});
function updateInfoBox4(content) {
    infoBox4.innerHTML = content;
}

// Helper function to create a custom marker element
function createMarkerElement(iconUrl) {
    const marker = document.createElement('div');
    marker.className = 'custom-marker';
    marker.style.backgroundImage = `url(${iconUrl})`;
    marker.style.width = '40px';
    marker.style.height = '40px';
    marker.style.backgroundSize = 'cover';

    return marker;
}
// updateInfoBox4("<h2>Product</h2><p> <b>CLEAR HDPE  </b> </p>");
// map4.addControl(new mapboxgl.NavigationControl());
// map4.addControl(new mapboxgl.FullscreenControl());
